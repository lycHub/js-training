...
上面Hero函数实例化了两个对象，两个对象都能调用sayHi函数，这很正常，但是仔细看下构造函数会发现，每个英雄除了有不同的属性或方法外（比如每个英雄的名字，年龄都不同），还可能存在相同的属性或方法，**每实例化一次，构造函数就会调用一次，并且会对函数中的每个成员开辟一个储存空间**
...
可以看到，虽然sayHi方法是长的一样，每个实例化出来的英雄都会有，但是他们并不相等，也就是说，sayHi方法占了两个内存空间，像这种每个实例都会有的共同属性或方法，占这么多空间显然是不合适，所以下面就用到原型来解决这个问题。
...
dema.sayHi();               // hi德玛
```
上面实例化了一次Hero并赋给变量dema，dema能访问到age很正常，因为age明显是挂在this上的，但是为什么dema能访问到挂在原型上的方法呢，可以打印一下dema, 结果如下：
<br /><br /><br />
...
然后再打印下Hero.prototype里的成员，还会发现有个constructor属性，这个属性的值就是Hero本身,
...
通过上面三段实例得出结论：**当访问实例上某个成员时，先在实例本身查找，找不到再去实例的原型属性(dema.__proto__)上去找，还找不到又会去父级的原型属性上找，知道找到Object.prototype为止**